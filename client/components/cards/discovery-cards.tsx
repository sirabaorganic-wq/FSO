...
export function ProductCard({ product }: { product: DiscoveryProduct }) {
  const href = product.slug === 'cold-pressed-groundnut-oil' ? `/shop/product/${product.slug}` : `/search?q=${encodeURIComponent(product.name)}`
  return <article className="group flex flex-col gap-4">
    <div className="relative aspect-[4/5] overflow-hidden bg-muted">
      ...
      <span className="absolute bottom-3 left-3 bg-background/90 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-foreground">{product.tags[0]}</span>
    </div>
